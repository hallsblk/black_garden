...
func _camera_control(event):
	if event is InputEventMouseMotion:
		...
		pivot.rotation.x -= event.relative.y * 0.003
		var limitRadians = deg_to_rad(40)
		pivot.rotation.x = clampf(pivot.rotation.x,-limitRadians,limitRadians)
...
func _set_animation(direction):
	if direction:
		...
		skin.rotation.y = lerp_angle(skin.rotation.y, targetAngle, 0.1)
